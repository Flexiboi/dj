...
-- M-DEVELOPMENT23: https://github.com/M-DEVELOPMENT23/M-PropV2/tree/main
...
local function textUILoop()
    CreateThread(function()
        while gizmoEnabled do
            Wait(100)
            lib.showTextUI(
                locale("gizmo.current_mode", currentMode, (isRelative and 'Relative' or 'World')) ..
                GetVectorText("coords") .. '  \n' ..
                GetVectorText("rotation") .. '  \n' ..
                '[G]     - ' .. (isCursorActive and locale("gizmo.disable_cursor") or locale("gizmo.enable_cursor")) .. '  \n' ..
                '[W]     - ' .. locale("gizmo.translate_mode") .. '  \n' ..
                '[R]     - ' .. locale("gizmo.rotate_mode") .. '  \n' ..
                '[Q]     - ' .. locale("gizmo.toggle_space") .. '  \n' ..
                '[LALT]  - ' .. locale("gizmo.snap_to_ground") .. '  \n' ..
                '[ENTER] - ' .. locale("gizmo.done_editing") .. '  \n'
            )
        end
        lib.hideTextUI()
    end)
...
CreateThread(function()
    Wait(1000)
    lib.callback("flex_dj:server:isAdmin", false, function(admin)
        if admin then
...
function use3dselect()
    ...
    while selecting do
        ...
        if hit then
            ...
            DrawSphere(coords.x, coords.y, coords.z, 0.02, 0, 255, 0, 0.8)
...
exports('use3dselect', use3dselect)